  ...
  return (
    <div className="flex flex-col h-full border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden group hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex-1 flex flex-col p-5">
        ...
        {/* Author and Tags */}
        <div className="flex flex-wrap justify-between items-center mt-auto pt-3 border-t border-gray-100 dark:border-gray-700">
          <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
            <FaUser className="mr-1" size={12} />
            <span>{item.author?.name || item.authorId?.name || "Admin"}</span>
          </div>
          ...
          {item.tags && item.tags.length > 0 && (
            <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
              ...
              {item.tags.length > 1 && <span> +{item.tags.length - 1}</span>}
            </div>
          )}
        </div>
